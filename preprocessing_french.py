...
base_path = "/home/clotilde/Documents/Cours/S2/Computational Linguistics/Project/Data/French"
feature_df, embeddings, labels, filenames = process_documents_by_genre(base_path, lang="en")

# Make sure results folder exists
os.makedirs("results_french", exist_ok=True)

# Save structural features to CSV
feature_df["genre"] = labels
feature_df["filename"] = filenames
feature_df.to_csv("results_french/structural_features.csv", index=False)

# Save embeddings to .npy (NumPy binary format)
np.save("results_french/doc_embeddings.npy", embeddings)

# Optional: Save labels separately
labels.to_csv("results_french/labels.csv", index=False)

# Load features and labels
features_df = pd.read_csv("results_french/structural_features.csv")
...
with open("results_french/top_features_per_genre.txt", "w", encoding="utf-8") as f:
...
# Save to results_french folder
plt.savefig("results_french/confusion_matrix.png")
plt.close()
print("✅ Confusion matrix saved to results_french/confusion_matrix.png")